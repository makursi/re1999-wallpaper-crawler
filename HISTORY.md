...
- **捕获量持续下滑（972 → 502 → 21）**：08-14 基准 972（net 998/dom 995/shh 234374），09-03 已降至 502（net 528/dom 525/shh 119704），2026-09-04 实测仅 21（net 47/dom 45/shh 4964）。2026-09-04 的两次诊断确认**根因在官网侧，非 crawler**：同代码同配置（run_meta 逐项一致）；页面静态检查只有 15 个 `.holder-img`、无 tab/无加载更多、列表高~850×5；壁纸 URL 止于 `20260729/976 版本1440x2560`（官网当前仅展示这批）；磁盘 971 文件含全部，下载侧全 Content-hash skip 干净。2026-09-04 加入 slow-network 修复（waitForList + 无条件滚动探测 + 900s 超时）并验证工作正常，但无法回归基线——官网没有更多可抓。后续跟踪（历史记录于 2026-06-18、2026-09-03、2026-09-04）。
- **慢网行为已修复但待长期观察**：2026-09-04 修复前两代（elapse<45 冻结滚动）在慢网收敛到 22；修复后探探测持续直到真 idle。等待官网内容恢复后用一次正常延迟的重跑验证 combinedCount 回到 ~500 基线。